...
global selected_file #making the selected file as global

def update(*args):
    date_label.config(text="Selected date is "+date_var.get())

def select_file():
    global selected_file #accessing the global variable
...
def save_to_excel(text, date):#defining function to add names and date in excel sheet
    # Check if a file has been selected earlier
...
submit=tk.Button(root,text="Submit",width=20,height=2,bg="black",fg="white",activebackground="blue",activeforeground="white",command=submit_to_excel)
submit.pack(pady=10)
